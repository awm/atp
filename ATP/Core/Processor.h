...
#include "ATP/Common/Dictionary.h"
...
Invoked to run the processor.  The processor takes in a dictionary (which may be NULL), and uses its contents to produce another dictionary
(which may also be NULL).  The processor may also use the command line parameters passed to it in <ATP_ProcessorLoadCallback>, and data loaded
...
Parameters:
    ...
    p_input  - The dictionary of input data for the processor.  This may be NULL.
    p_output - A pointer to populate with the dictionary of output data from the processor.  The referenced pointer may be set to NULL
               (i.e. *p_output = NULL).
...
*/
typedef int (*ATP_ProcessorRunCallback)(int p_count, const ATP_Dictionary *p_input, ATP_Dictionary **p_output, void *p_token);
...
Parameters:
    ...
    p_count  - The total number of processors loaded.
    p_input  - A dictionary containing all of the input data for the processor.  This may be NULL if the processor is the first in
               the pipeline or the previous processor produced no output.
    p_output - A pointer to set to the output dictionary generated by the processor, if any.  The processor may set this to NULL if
